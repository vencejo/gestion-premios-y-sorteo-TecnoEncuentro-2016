...
class EquipoCompeticion(Equipo):
    ...
    def __init__(self, workBook, nombre):
        ...
        tiposPrueba = []
        tiposPrueba.append(self.equipo['G2'].value.lower())
        tiposPrueba.append(self.equipo['G3'].value.lower())
        tiposPrueba.append(self.equipo['G4'].value.lower())
        self.puntos = self.equipo['F2'].value + self.equipo['F3'].value + self.equipo['F4'].value
        # Si hay dos tipos de prueba distintos habrá bonus de 300 puntos
        if len(set(tiposPrueba)) == 2:
            self.puntos += 300     
